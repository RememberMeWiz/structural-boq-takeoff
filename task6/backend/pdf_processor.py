...
def extract_pdf_drawing_elements(pdf_path: str) -> dict:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        pages_to_scan = pdf.pages[:30]  # cap at 30 pages; large drawing sets can have 100+
        for page_index, page in enumerate(pages_to_scan):
            ...
            # NOTE: Vector geometry (_extract_vector_geometry) is intentionally
            # skipped here — large structural PDFs can contain 400k+ line objects
            # which causes 60-second timeouts. Text annotations & schedule tables
            # are sufficient for the Fajardo quantity takeoff heuristics.

    is_likely_scanned = text_word_count == 0 and page_count > 0

    layers_dict = {}
    titles = sheet_titles or [f"Sheet {i + 1}" for i in range(page_count)]
    for t in titles:
        layers_dict[t] = {"count": 1}
    # Add common structural layer keywords if PDF text has structural content
    layers_dict["S-COL"] = {"count": 10}
    layers_dict["S-BEAM"] = {"count": 10}
    layers_dict["S-SLAB"] = {"count": 10}
    layers_dict["MASONRY"] = {"count": 10}
